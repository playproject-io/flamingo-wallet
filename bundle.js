...
async function action_buttons (opts = {}, protocol) {
  ...
  const sendButton = await generalButton(subs[0], button_protocol('send_general'))
  const receiveButton = await generalButton(subs[1], button_protocol('receive_general'))
  const walletButton = await generalButton(subs[2], button_protocol('wallet_general'))
